...
const TodoListOrder := preload("res://scripts/todos/todo_list_order.gd")
...
func test_ordered_active_first_preserves_groups() -> void:
	var items: Array = [
		_make_todo(1, DbConstants.TODO_DONE, 0),
		_make_todo(2, DbConstants.TODO_PENDING, 1),
		_make_todo(3, DbConstants.TODO_DONE, 2),
		_make_todo(4, DbConstants.TODO_IN_PROGRESS, 3),
	]
...
func test_apply_sort_orders_renumbers() -> void:
	var items: Array = [
		_make_todo(1, DbConstants.TODO_PENDING, 5),
		_make_todo(2, DbConstants.TODO_DONE, 9),
	]
